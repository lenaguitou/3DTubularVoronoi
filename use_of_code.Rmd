...
filename <- "results/static/results_simulation_25_01_17_3.RData"
...
frame =  parameters$n_steps
points <- filter(result_alg$points_evolution, Frame == frame)
...
Parallel execution to execute multiple simulations at the same time.
...
for(i in 1:N_SIM){
  filename <- paste0("results/parallel/results_simulation_25_01_19_",i,".RData")
...
`{r}{r include=FALSE} # Access information about the simulation 3 hist_points_sim_3 = par_results[[3,1]]$points_evolution`
...
selected_frame = par_parameters$n_steps
# Average number of neighbors in the apical area 
edgear_filteredA <- dplyr::filter(edgearsim[[1]], Frame == selected_frame)
stationarylewisApical(edgear_filteredA[1:(par_parameters$n_cells),c(1,2,3)])  
# Average number of neighbors in the basal area
edgear_filteredB <- dplyr::filter(edgearsim[[2]], Frame == selected_frame)
stationarylewisBasal(edgear_filteredB[1:(par_parameters$n_cells),c(1,2,3)]) 
```

An error message can appear that comes from the instability of the regression function. If it happens the function doesnt work, right click on this chunk, "reload" and run it again.

```{r}
PATH_SIMANALYSIS = "tubular_simulations_source/source_simulation_analysis.R"
source(PATH_SIMANALYSIS)

# Average energy relaxation and regression
adjsim(par_results,nsim=N_SIM,it=par_parameters$n_steps)
```

The function "energy_analisis_averages" can be quite long => set to take max 50 simulations. 

```{r}
PATH_ANALYSIS = "tubular_simulations_source/source_simulation_analysis.R"
source(PATH_ANALYSIS)

# Average energies relaxation over time
enerhist_average <- energy_analisis_averages_par(par_results, it = par_parameters$n_steps, lay = par_parameters$n_layers, n = par_parameters$n_cells,rad_coef = par_parameters$ratio_rad,Radius = par_parameters$apical_rad,cylen = par_parameters$cyl_length,gamma = par_parameters$gamma,lambda=par_parameters$lambda )
```

The function "scutoids_percr_simulations_par" can be quite long => set to take max 50 simulations. 
...
frame <- par_parameters$n_steps
scutoids_analysis_simulations(par_results, it = frame , sim = N_SIM, n = par_parameters$n_cells,cylen = par_parameters$cyl_length,ap_rad = par_parameters$apical_rad, Ratio = par_parameters$ratio)

# Scutoids percentage over time (parallel execution)
scutoids_percr_simulations_par(par_results, n=par_parameters$n_cells, sim=N_SIM, it= frame,ap_rad = par_parameters$apical_rad, cylen=par_parameters$cyl_length,ratio=par_parameters$ratio_rad)
