...
```{r}
simulation <- perform_simulations(algorithm="static", parameters=params, return_params=TRUE)
...
result_alg_bending <- perform_simulations(algorithm="bending", parameters=params_bending)
```
...
To load the results:
```{r}
# CHANGE file_name WITH THE CORRESPONDING PATH (run the perform_simulations function with save=TRUE, and check the folder results)
...
results_par<-foreach(i=c(1,2), .combine = rbind, .packages = "deldir") %dopar% {
  ...
  simulation_params <- list(algorithm="static", parameters=parameters, save=FALSE)
...
We recover the parameters used before.
```{r}
rad_coef = params$ratio_rad
steps = params$n_steps
Radius = params$apical_rad
