...
Load previous results. Edit *fileName*.

```{r}
fileName <- "results/static/results_simulation_25_01_31.RData"
...
#apical area
stationarylewisApical(edgear[[1]][1:parameters$n_cells,c(1,2,3)],it=selectedIteration)
#basal area
stationarylewisBasal(edgear[[2]][1:parameters$n_cells,c(1,2,3)],it=selectedIteration)
...
Load pre-existing results. Edit the file name and the number *N_SIM* of simulations to import.

```{r}
fileName <- "results/parallel/results_simulation_25_01_19_"
N_SIM = 10 
...
selected_frame =  35# par_parameters$n_steps
# Average number of neighbors in the apical area 
edgear_filteredA <- dplyr::filter(edgearsim[[1]], Frame == selected_frame)
stationarylewisApical(edgear_filteredA[1:(par_parameters$n_cells),c(1,2,3)],it = selected_frame)  
# Average number of neighbors in the basal area
edgear_filteredB <- dplyr::filter(edgearsim[[2]], Frame == selected_frame)
stationarylewisBasal(edgear_filteredB[1:(par_parameters$n_cells),c(1,2,3)],it = selected_frame) 
```
...
# Average energy relaxation of the tesselations and regression
adjsim(par_results,nsim=N_SIM,it=par_parameters$n_steps)
```

The function "energy_analisis_averages" can be quite long that is why it is built in parallel.
...
The function "scutoids_percr_simulations_par" can be quite long that is why it is built in parallel.
...
frame <- 35 # par_parameters$n_steps
...
saveFrame <- par_parameters$n_steps # last frame is parameters$n_steps
...
points <- filter(par_results[[1]]$points_evolution, Frame == saveFrame)
