...
```{r}
list(result_alg, parameters, algorithm) <- perform_simulations(algorithm="static", parameters=params)
```

Simulations with bending:
```{r}
result_alg_bending <- list(result_alg_bending, parameters, algorithm) <- perform_simulations(algorithm="bending")
```
...
time <- format(Sys.time(), "%Y_%m_%d__%H_%M")


get_results <- function(path){
  fromJSON(path)
  points_evolution <- as.data.frame(results$points_evolution)
  energy_evolution <- as.data.frame(results$energy_evolution)
  nu_2 <- results$nu_2
  parameters <- results$parameters_used
  algorithm_used <- results$algorithm_used
  time_of_execution <- results$time_end_execution
  return(c(points_evolution, energy_evolution, nu_2, parameters, algorithm_used,
           time_of_execution))
}
...
```{r,include=FALSE}
PATH_ANALYSIS = "∼/3DTubularVoronoi/Source/source_analysis.R"
...
```{r,include=FALSE}
PATH_SIMANALYSIS = "∼/3DTubularVoronoi/Source/source_simulation_analysis.R"
